...
void initState(const enum GAME_STATE state) {
    ...
    switch(state) {
        case START_SCREEN:
            addEntity(PLAYER_STAND_TID, 110, GROUND_OFFSET, PLAYER);
            break;
    ...
    switch (roomLevel) {
        case 0:
            if (spawn_wave & 1) { // slow down the spawns
                ...
                else addTallEnemy(SWAP_SIDE(spawn_wave));
            }
            break;
        case 1:
            if (spawn_wave & 3) {
...
INLINE void runner_checkPlayerCollisions() {
    ...
    while (i < objs_length) {
        ...
        int ent_x_noOverflow = ent_x;
        if (ent_x > (int)SCREEN_WIDTH + 10) {
            ent_x_noOverflow -= (int)(0xFF + 10);    // yesss finally figured this bs out -- fix screen wrap around by manually subtracting 1 ubyte
        } 
        ...
        if (player_y < obstacle_attrs.height - fix_sprite_height_a_little && player_x < ent_x_noOverflow + 64 && player_x > ent_x_noOverflow) {
            gameOver();
        }
        if (ent_x > SCREEN_WIDTH + 1) { // entity has wrapped around -- remove it
            score++;
            removeEntity(i--);
        }
        i++;
    ...
    INLINE void shooter_checkPlayerCollisions() {
        ...
        while (engager_i < objs_length) {
            ...
            while (i < objs_length) {
                ...
                if (engager_type == PLAYER) {
                    ...
                    if (engager_y - ent_y > engager_width - 4 && engager->dy <= 0 
        ...
        if (playerHurt_tribool && PLAYER_ENTITY->f_invuln == 0) { // the player is newly hurt in this frame
            if (hurtEntity(PLAYER_ENTITY, 1, playerHurt_tribool)) {
                ...
                PLAYER_ENTITY->f_invuln = 10;
            }
            PLAYER_ENTITY->dx = playerHurt_tribool * 3;
        } 
    }

    uint player_ekilledInCurrJump;
    void tickEntities(const uint count) {
        if (!(count & 3)) {
            for (int i = 0; i < objs_length; i++) {
                ...
                if (gameState == RUNNER || gameState == START_SCREEN_NODRAW) {
